...
class BulkAjustarExistenciasNegativasRequest extends FormRequest
{
    public function authorize(): bool
    {
        $user = $this->user();

        return $user && ($user->tienePermiso('inventario_base.entrada') || $user->tienePermiso('inventario_base.ajustar'));
    }

    public function rules(): array
    {
        return [
            'row_keys' => ['required', 'array', 'min:1', 'max:100'],
            'row_keys.*' => ['required', 'string', 'regex:/^\d+:\d+$/'],
    ...
    {
        return [
            'row_keys.required' => 'Debes seleccionar al menos un registro.',
            'row_keys.array' => 'La selección enviada no tiene un formato válido.',
            'row_keys.min' => 'Debes seleccionar al menos un registro.',
            'row_keys.max' => 'Solo puedes ajustar hasta 100 registros por operación.',
            'row_keys.*.regex' => 'Uno de los registros seleccionados no es válido.',
