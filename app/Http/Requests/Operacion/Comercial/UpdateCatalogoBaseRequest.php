...
class UpdateCatalogoBaseRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $nombreRule = $tipo === 'categorias'
            ...
            : [
                ...
                'string',
                'max:120',
                Rule::unique($config['table'], $config['nombre'])
                    ->ignore($id, $config['id'])
                    ->where(fn ($query) => $query->where($config['deleted'], false)),
            ];
        ...
        $rules = [
            ...
            'clave' => [
                ...
                'string',
                'max:40',
                Rule::unique($config['table'], $config['clave'])
                    ->ignore($id, $config['id'])
                    ->where(fn ($query) => $query->where($config['deleted'], false)),
            ],
        ...
        if ($tipo === 'unidades') {
            $rules['codigo'] = [
                ...
                Rule::unique($config['table'], 'umd_codigo')
                    ->ignore($id, $config['id'])
                    ->where(fn ($query) => $query->where('umd_deleted', false)),
            ];
    ...
    public function withValidator($validator): void
    {
        ...
        $validator->after(function ($validator) use ($id): void {
    ...
    {
        return match ($tipo) {
            'marcas' => ['table' => 'tbl_marcas_mrc', 'id' => 'mrc_id', 'nombre' => 'mrc_nombre', 'clave' => 'mrc_clave', 'deleted' => 'mrc_deleted'],
