...
class OperacionCatalogoComercialController extends Controller
{
    ...
    public function dataProductos(Request $request): JsonResponse
    {
        ...
        $data = $productos->map(fn ($item) => [
            ...
            'costo_minimo_sku' => $item->costo_minimo_sku,
            'costo_maximo_sku' => $item->costo_maximo_sku,
    ...
    public function dataSkus(Request $request): JsonResponse
    {
        ...
        $data = $skus->map(function ($item): array {
            ...
            return [
                ...
                'marca_nombre' => $item->producto?->marca?->mrc_nombre,
                'modelo_nombre' => $item->producto?->modelo?->mdl_nombre,
                'concepto_nombre' => $item->producto?->categoria?->ctg_nombre,
                'descripcion_catalogo' => $item->producto?->descripcionCatalogo?->dsc_nombre,
    ...
    private function catalogTypes(): array
    {
        return [
            ...
            ['key' => 'descripciones', 'label' => 'Descripción'],
    ...
    private function productoFormViewData(?Producto $producto = null): array
    {
        ...
        return [
            ...
            'opciones' => [
                ...
                'descripciones' => $opcionesProducto['descripciones'],
            ...
            'categoriasCatalogo' => $opcionesProducto['categorias']
                ...
                ->values(),
            'descripcionesCatalogo' => $opcionesProducto['descripciones']
                ->map(fn ($item) => [
                    'dsc_id' => $item->dsc_id,
