...
class ReporteConsultaService
{
    ...
    {
        foreach ($this->catalogo() as $grupo => $categoria) {
            foreach ($categoria['reportes'] as $reporte) {
                if ($reporte['slug'] === $slug) {
                    return [...$reporte, 'categoria' => $grupo, 'categoria_titulo' => $categoria['titulo']];
                }
            }
        }
    ...
    private function ventasAgrupadas(string $tipo, int $sucursal, Carbon $desde, Carbon $hasta, array $f, bool $exportar): array
    {
        ...
        if ($tipo === 'vendedor') {
            $q->selectRaw('ROUND(SUM(importe) / NULLIF(COUNT(DISTINCT folio),0),2) as ticket_promedio');
        }
        if ($tipo === 'producto') {
            $q->selectRaw('ROUND(SUM(importe) / NULLIF(SUM(cantidad),0),2) as precio_promedio');
        }
        if (in_array($tipo, ['categoria', 'modelo', 'marca', 'linea'], true)) {
            $total = DB::query()->fromSub(clone $mov, 't')->sum('importe');
            $q->selectRaw('ROUND((SUM(importe) / NULLIF(?,0))*100,2) as participacion', [$total]);
        }
    ...
    private function ventasMetodosPago(int $sucursal, Carbon $desde, Carbon $hasta, array $f): array
    {
        ...
        $acum = [];
        foreach ($ventas as $v) {
            $detalle = json_decode((string) $v->psv_pago_detalle, true) ?: [];
            if ($v->psv_metodo_pago === 'mixto' && $detalle) {
                foreach (['efectivo', 'tarjeta', 'monedero_electronico'] as $m) {
                    $monto = (float) ($detalle[$m] ?? 0);
                    if ($monto > 0) {
                        $acum[$m]['monto'] = ($acum[$m]['monto'] ?? 0) + $monto;
                        $acum[$m]['tickets'] = ($acum[$m]['tickets'] ?? 0) + 1;
                    }
                }
            } else {
                $m = $v->psv_metodo_pago ?: 'sin_especificar';
                $acum[$m]['monto'] = ($acum[$m]['monto'] ?? 0) + (float) $v->psv_total;
                $acum[$m]['tickets'] = ($acum[$m]['tickets'] ?? 0) + 1;
            }
        }
        $total = array_sum(array_column($acum, 'monto'));
        $rows = collect($acum)->map(fn ($x, $m) => (object) ['metodo' => str($m)->replace('_', ' ')->title(), 'tickets' => $x['tickets'], 'monto' => round($x['monto'], 2), 'participacion' => $total ? round($x['monto'] / $total * 100, 2) : 0])->sortByDesc('monto')->values();
    ...
    private function resultado(array $headers, Collection $rows, array $kpis): array
    {
        return ['encabezados' => $headers, 'rows' => $rows->values(), 'kpis' => collect($kpis)->map(fn ($v) => is_numeric($v) ? round((float) $v, 2) : $v)->all(), 'total_registros' => $rows->count()];
    }

    private function rango(array $f): array
    {
        $desde = Carbon::parse($f['desde'] ?? now()->toDateString());
        $hasta = Carbon::parse($f['hasta'] ?? now()->toDateString());
    ...
    private function def(string $slug, string $titulo, string $descripcion, string $icono, string $permiso): array
    {
        return compact('slug', 'titulo', 'descripcion', 'icono', 'permiso');
    }
}
